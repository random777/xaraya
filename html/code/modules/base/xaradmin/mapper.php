    ...
    switch ($data['tab']) {
        ...
        case 'encode':
            ...
            if (!empty($encode_module)) {
                $encoded = xarController::getRouter()->encode(new xarUrl2($encode_module, $encode_type, $encode_func, $encode_args, xarServer::$generateXMLURLs, $encode_target, $encode_entrypoint));
                $data['encoded'] = $encoded->getParams();
                $data['encoded']['url'] = $encoded->getUrl();
                $data['encoded']['route'] = $encoded->getEncoder();
            }
