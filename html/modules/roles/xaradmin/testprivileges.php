...
function roles_admin_testprivileges()
{
    ...
    if (!xarVarFetch('pmodule', 'int', $module, Privileges_Masks::PRIVILEGES_ALL, XARVAR_NOT_REQUIRED,XARVAR_PREP_FOR_DISPLAY)) return;
    ...
    if (!empty($test)) {
        ...
        $mask = Privileges_Masks::getMask($name);
        $component = $mask->getComponent();
        // test the mask against the role
        $testresult = Privileges_Masks::xarSecurityCheck($name, 0, $component, 'All', $mask->getModule(), $role->getName());
        // test failed
        if (!$testresult) {
        ...
        // test returned an object
            ...
            $data['rlevel'] = Privileges_Masks::$levels[$thisprivilege->getLevel()];
        }
        ...
        foreach ($testmasks as $testmask) {
            $thismask = array('sname' => $testmask->getName(),
                ...
                'slevel' => Privileges_Masks::$levels[$testmask->getLevel()]
                );
    ...
    $data['masks'] = Privileges_Masks::getmasks($module);
