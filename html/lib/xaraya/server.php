 ...
 * Deprecation Row
 */
function xarServerGetCurrentURL($args = array(), $generateXMLURL = NULL, $fragment = NULL) {return xarServer::getCurrentURL($args, $generateXMLURL, $fragment);}
function xarServerGetBaseURL($secure = false) {return xarServer::getBaseURL($secure);}
function xarServerGetProtocol($secure = false) {return xarServer::getProtocol($secure);}
...
/* Jamaica version
        try {
            $BaseURI =  xarCore_getSystemVar(sys::LAYOUT, 'BaseURI');
            return $BaseURI;
        } catch(Exception $e) {
            // We need to build it
        }
        ...
        if (!empty($modName)) {
            ...
            if (!empty($objectName)) {
                ...
                if (empty(self::$defaultRequestInfo)) {
                    self::$defaultRequestInfo = array(xarModGetVar('modules', 'defaultmodule'),
                                                      xarModGetVar('modules', 'defaultmoduletype'),
                                                      xarModGetVar('modules', 'defaultmodulefunction'));
                }
