...
// {{{ Header
/*
 * -File       $Id: File.php,v 1.32 2003/07/09 06:06:39 purestorm Exp $
...
import('phing.system.lang.NullPointerException');
 ...
 * FIXME
 *  - catch error
 *  - substr for example returns false if an index given is out of bounds
 *    the hystack string. This is not acceptable as it will lead to unexpected
 *    results. Those kind of errors should be catched to guarantee save execution
 *
...
class File {
    ...
    /** constructor */
    function File($arg1 = null, $arg2 = null) {

        static $__dsep;
        static $__psep;

        // emulates a kinda private static "reference" to a singelton
        $fs =& FileSystem::getFileSystem();

        if (!$__dsep) {
            $__dsep = $fs->getSeparator();
        }
        if (!$__psep) {
            $__psep = $fs->getPathSeparator();
        }
        $this->separator =& $__dsep;
        $this->pathSeparator =& $__psep;

        /* simulate signature identified constructors */
        if (is_a($arg1, "File") && is_string($arg2)) {
        ...
        elseif(is_string($arg1) && is_string($arg2)) {
            ...
            if ($arg1 === null) {
                throw (new NullPointerException("Argument1 to function must not be null", __FILE__, __LINE__));
                System::Halt(-1);
                return;
            }
    ...
    function _constructPathname($pathname) {
        ...
        if ($pathname == null) {
            throw (new NullPointerException("Argument to function must not be null"), __FILE__, __LINE__);
            return;
        }
    ...
    function getName() {
        ...
        $index = ((($res = strrpos($this->path, $this->separator)) === false) ? -1 : $res);
    ...
    function _slashify($path, $isDirectory) {
        ...
        if ($this->separator !== '/') {
            $p = str_replace($this->separator, '/', $p);
        }
     ...
     */
    function exists() {
        clearstatcache(); // Clear PHP's fs cache
        if ($this->isFile()) {
    ...
    function isDirectory() {
        ...
        if ($fs->checkAccess($this) !== true) {
            die("No read access to ".$this->path."\n");
        }
        return @is_dir($this->path);
    }
    ...
    function mkdir() {
        ...
        if ($fs->checkAccess(new File($this->path), true) !== true) {
            die("No write access to " . $this->path."\n");
        }
     ...
     */
    function renameTo(&$destFile) {
        $fs =& FileSystem::getFileSystem();
        if ($fs->checkAccess($this) !== true) {
            die("No write access to ".$this->getPath());
        }
     ...
     * @return  true if and only if the renaming succeeded; false otherwise
     */
    function copyTo(&$destFile) {
        $fs =& FileSystem::getFileSystem();

        if ($fs->checkAccess($this) !== true) {
            die("No read access to ".$this->getPath()."\n");
        }

        if ($fs->checkAccess($destFile, true) !== true) {
            die("File::copyTo() No write access to ".$destFile->getPath());
        }
    ...
    function setLastModified($time) {
        $time = (int) $time;
        if ($time < 0) {
            die("IllegalArgumentException, Negative $time\n");
        }

        // FIXME check if accessible
        $fs =& FileSystem::getFileSystem();
        if ($fs->checkAccess($this, true) !== true) {
            die("File::setLastModified(). No write access to file\n");
    ...
    function &createTempFile($prefix, $suffix, &$directory) {
        ...
        $fs =& FileSystem::getFileSystem();
        // TODO: catch and rethrow Exception occurring in FileSystem::createNewFile()
    ...
    function equals(&$obj) {
        if (($obj !== null) && (isInstanceOf($o, 'File'))) {
