<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:output method="html" encoding="iso-8859-1" omit-xml-declaration="no"/>

    <!-- Set of parameters which are passed in by the XSLT transform module -->
    <xsl:param name="xar_docuri"/>
    <xsl:param name="xar_webpath"/>
    ...
    <xsl:template match="eref[node()]">
        <!-- INSERT poor mans detection for relative / absolute links HERE -->
       
        <xsl:choose>
            <xsl:when test="contains('http://',@target)">
                <a href="{@target}"><xsl:apply-templates /></a>
            </xsl:when>
            <xsl:otherwise>
                <a href="{$xar_webpath}/{@target}"><xsl:apply-templates /></a>              
            </xsl:otherwise>
        </xsl:choose>
        ...
        <td valign="top">
            ...
            <xsl:choose>
                <xsl:when test="string-length($target) &gt; 0">
                    <xsl:text>"</xsl:text><a href="{$xar_docuri}{$target}"><xsl:value-of select="front/title" /></a><xsl:text>"</xsl:text>
                </xsl:when>
                <xsl:otherwise>
...
<xsl:template match="xref[node()]">
    ...
    <a href="{$xar_docuri}#{$target}"><xsl:apply-templates /></a>
...
<xsl:template name="emitheader">
    ...
    <xsl:for-each select="$lc/myns:item | $rc/myns:item">
        ...
        <tr>
            <td width="33%" class="header"><xsl:copy-of select="$lc/myns:item[$pos]/node()" />&#0160;</td>
            <td width="33%" class="header"><xsl:copy-of select="$rc/myns:item[$pos]/node()" />&#0160;</td>
        </tr>
    </xsl:if>
</xsl:for-each>
</xsl:template>
...
<xsl:template name="insertIndex">
    ...
    <a name="{$anchor-prefix}.index"><h1>Index</h1></a>
...
<xsl:template match="/" mode="toc">
    ...
    <h1><a name="{$anchor-prefix}.toc">Table of Contents</a></h1>
...
<!-- Insert a line in the table of contents -->
            ...
            <xsl:choose>
                <xsl:when test="starts-with($number,'del-')">
                    ...
                    <del>
                        ...
                        <a href="{$xar_docuri}#{$target}"><xsl:value-of select="$title"/></a>
                    </del>
                </xsl:when>
    ...
    <xsl:if test="$includeTitle or $includeToc='yes'">
        <table class="noprint" border="0" cellpadding="0" cellspacing="2" width="30" height="15" align="right">
            <xsl:if test="$includeTitle">
                <tr>
                    <td align="center" valign="center" width="30" height="30">
            ...
            <xsl:if test="$includeToc='yes'">
                <tr>
                    <td align="center" width="30" height="15">
                        <a href="{$xar_docuri}#{$anchor-prefix}.toc" CLASS="link2"><b class="link2">&#0160;TOC&#0160;</b></a>
                    </td>
                </tr>
            </xsl:if>
        </table>
...
<p>
    <table>
        <xsl:for-each select="//ed:issue">
            ...
            <tr>
                <td><a href="{$xar_docuiri}#{$anchor-prefix}.issue.{@name}"><xsl:value-of select="@name" /></a></td>
    ...
    <ins>
        ...
        <xsl:if test="@ed:resolves">
            <table style="background-color: khaki; border-width: thin; border-style: solid; border-color: black;" align="right">
                <tr><td>resolves: <a href="{$xar_docuri}#{$anchor-prefix}.issue.{@ed:resolves}"><xsl:value-of select="@ed:resolves"/></a></td></tr>
            </table>
        </xsl:if>
        <xsl:apply-templates />
    </ins>
